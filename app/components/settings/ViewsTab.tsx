...
import type { View, ViewsConfig, NodeId } from '@/lib/content/views';
...
export function ViewsTab({ initialConfig }: ViewsTabProps) {
  ...
  // Helper to get views from config (supports both 'views' and 'items' keys)
  const getViews = () => config.views || config.items || [];
  ...
  const saveConfig = async (newConfig: ViewsConfig) => {
    ...
    try {
      console.log('%c→ PUT%c /views', 'color: #eab308; font-weight: bold', '', newConfig);
      ...
      console.log('%c← 200%c /views (saved)', 'color: #22c55e', '');
      setConfig(newConfig);
      setSuccess(true);
  ...
  const handleCreateView = () => {
    setEditingView(createDefaultView(getViews()));
    setIsCreating(true);
  };
  ...
  const handleSaveView = async (view: View) => {
    ...
    if (isCreating) {
      newViews = [...currentViews, view];
    } else {
      newViews = currentViews.map((v) => (v.id === view.id ? view : v));
    }
  ...
  const handleDeleteView = async (viewId: NodeId) => {
    const newViews = getViews().filter((v) => v.id !== viewId);
    const newConfig: ViewsConfig = {
      ...config,
      views: newViews,
      defaultHomeViewId:
  ...
  const handleSetHome = async (viewId: NodeId) => {
    const newViews = getViews().map((v) => ({
      ...v,
      isHome: v.id === viewId,
  ...
  const handleReorder = async (orderedIds: NodeId[]) => {
    const viewsById = new Map(getViews().map((v) => [v.id, v]));
