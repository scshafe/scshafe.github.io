...
function NavItemEditModal({ item, views, onUpdate, onClose, showPosition }: NavItemEditModalProps) {
  ...
  const getLinkTargetDisplay = () => {
    switch (localLinkType) {
      case 'view': {
        const view = views.find(v => v.root_node_id === localViewId);
        ...
        <div className="p-4 space-y-4">
          ...
          {localLinkType === 'view' && (
            <div>
              ...
              <select
                value={localViewId ?? ''}
                onChange={(e) => setLocalViewId(e.target.value ? asNodeId(Number(e.target.value)) : undefined)}
                className={selectClass}
              >
...
export function NavItemEditor({
  ...
  const getSubtitle = () => {
    ...
    switch (linkType) {
      case 'view': {
        const view = views.find(v => v.root_node_id === item.viewId);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[length:var(--text-xs)] text-[var(--foreground-muted)] truncate">
                {getSubtitle()}
                <span className="opacity-60 ml-2">• id:{item.id}</span>
