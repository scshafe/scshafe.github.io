...
export function NavigationTab({ initialConfig }: NavigationTabProps) {
  ...
  // Drag state
  const [draggedItem, setDraggedItem] = useState<{ section: 'header' | 'footer'; id: string } | null>(null);
  const [dragOverItem, setDragOverItem] = useState<{ section: 'header' | 'footer'; id: string } | null>(null);
  ...
  const updateItem = (section: 'header' | 'footer', id: string, updates: Partial<NavItem>) => {
  ...
  const removeItem = (section: 'header' | 'footer', id: string) => {
    const newConfig = { ...config };
  ...
  // Drag handlers
  const handleDragStart = useCallback((section: 'header' | 'footer', id: string) => {
    setDraggedItem({ section, id });
  }, []);

  const handleDragOver = useCallback((e: React.DragEvent, section: 'header' | 'footer', id: string) => {
    e.preventDefault();
  ...
  const handleDrop = useCallback((e: React.DragEvent, section: 'header' | 'footer', targetId: string) => {
    e.preventDefault();
    setDragOverItem(null);
