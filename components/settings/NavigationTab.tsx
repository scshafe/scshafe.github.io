...
import type { ResolvedView, NodeId } from '@/lib/content/types';
import { generateId } from '@/lib/content/views';
import { NavItemEditor } from './NavItemEditor';
import { useAppDispatch } from '@/lib/store/hooks';
import { initializeNavigation } from '@/lib/store/slices/navigationSlice';
...
export function NavigationTab({ initialConfig }: NavigationTabProps) {
  ...
  const [views, setViews] = useState<ResolvedView[]>([]);

  // Log initial config on mount
  useEffect(() => {
    console.log('%c[NavigationTab] Initial config from server:', 'color: #f59e0b; font-weight: bold');
    console.log('  siteName:', initialConfig.siteName);
    console.log('  Header items:', initialConfig.header.length);
    initialConfig.header.forEach((item, idx) => {
      console.log(`    [${idx}] id=${item.id}, label="${item.label}", linkType=${item.linkType}, viewId=${item.viewId}, url=${item.url}`);
    });
    console.log('  Footer items:', initialConfig.footer.length);
    initialConfig.footer.forEach((item, idx) => {
      console.log(`    [${idx}] id=${item.id}, label="${item.label}", linkType=${item.linkType}, viewId=${item.viewId}, url=${item.url}`);
    });
  }, [initialConfig]);
  ...
  useEffect(() => {
    async function fetchViews() {
      try {
        console.log('%c→ GET%c /views?resolved=true (fetching views for nav dropdown)', 'color: #06b6d4; font-weight: bold', '');
        const res = await fetch(`${DEV_EDITOR_URL}/views?resolved=true`);
        if (res.ok) {
  ...
  const saveConfig = async (newConfig: NavigationConfig) => {
    ...
    try {
      ...
      setConfig(newConfig);
      // Update Redux store so Header/Footer reflect changes immediately
      dispatch(initializeNavigation(newConfig));
      setSuccess(true);
