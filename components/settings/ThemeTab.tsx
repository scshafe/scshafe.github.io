...
import { useTheme, applyThemeColors } from '@/components/ui/ThemeProvider';
...
export function ThemeTab({ initialConfig, onThemeChange }: ThemeTabProps) {
  ...
  const saveConfig = useCallback(async (newConfig: ThemeConfig) => {
    ...
    try {
      console.log('%c→ PUT%c /themes', 'color: #eab308; font-weight: bold', '', newConfig);
      ...
      if (!res.ok) {
        console.log(`%c← ${res.status}%c /themes (failed)`, 'color: #ef4444', '');
        throw new Error('Failed to save');
      }

      console.log('%c← 200%c /themes (saved)', 'color: #22c55e', '');
      setConfig(newConfig);
      setSuccess(true);
