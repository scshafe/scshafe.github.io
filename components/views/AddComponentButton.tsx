...
import type { ViewComponentType, ViewComponent } from '@/lib/content/views';
...
export function AddComponentButton({ existingComponents, viewId, onAdd }: AddComponentButtonProps) {
  const [isOpen, setIsOpen] = useState(false);

  const componentOptions = getComponentTypeOptions();

  const handleSelect = (type: ViewComponentType) => {
    const newComponent = createDefaultComponent(type, existingComponents, viewId);
    // Add parentId to the new component
    newComponent.parentId = viewId;
    onAdd(newComponent);
    setIsOpen(false);
  };
