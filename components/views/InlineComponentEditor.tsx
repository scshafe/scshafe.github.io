...
/**
 * InlineComponentEditor - Stub for editing components inline.
 *
 * This component needs to be rewritten to use the new entity architecture
 * with ResolvedNode and proper API calls.
 *
 * TODO: Implement proper inline editing with:
 * - ResolvedNode type
 * - API calls to update components via references or direct component updates
...
interface InlineComponentEditorProps {
  ...
  onSave: (node: ResolvedNode, config: Record<string, unknown>) => Promise<void>;
  onCancel: () => void;
}
      ...
      <div className="bg-[var(--background)] border border-[var(--border)] rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="p-4 bg-[var(--background-secondary)] border border-[var(--border)] rounded-lg">
            ...
            <p className="text-[var(--foreground-muted)] text-[length:var(--text-sm)] mt-2">
              Node ID: <code className="text-[var(--foreground)]">{component.node_id}</code>
            </p>
            <p className="text-amber-400 text-[length:var(--text-sm)] mt-4">
              Inline component editing is under development. For now, please edit components using the direct config updates.
            </p>
          </div>
        </div>
